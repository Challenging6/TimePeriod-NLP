...
public class TimeUnit {
	...
	public void norm_sethour() {
		 ...
		 * 1.中午/午间0-10点视为12-22点 2.下午/午后0-11点视为12-23点 3.晚上/傍晚/晚间/晚1-11点视为13-23点，12点视为0点
...
//            System.out.println("this: "+Arrays.toString(_tp.tunit));
	 ...
	 * @date 2019年7月18日 下午5:10:17
	 * @param modifiedType
	 */
	public void modifyUnit(String modifiedType) {
		Calendar c = Calendar.getInstance();
		c.setTime(this.time);
		if (modifiedType.equals("year-1")) {
			// 往前算一年
			c.add(Calendar.YEAR, -1);
			this.setTime(c.getTime());
			// 修改时间标准表达
			String timeNorm = this.Time_Norm;
			...
			while (matcher.find()) {
				...
				this.Time_Norm = timeNorm;
			}
			// 修改时间修正标志位
			this.setModified(true);
			this.modifiedType = "year-1";
		} else if (modifiedType.equals("month-1")) {
			// 往前算一个月
	 ...
	 * 
	 * @author Xianjie Wu wuxianjie@ai-strong.com
	 * @date 2019年7月19日 上午1:15:06
	 * @param exp_time
	 * @return
	 */
	private String recoverSpTime(String exp_time) {
		// 初始化特殊时间角标
		int firstNum = -1;
		int secondNum = -1;
		String pattern = "#sptime(?<first>[0-9]{1,2})#(?<second>[0-9]{1,2})#";
		Matcher matcher = Pattern.compile(pattern).matcher(exp_time);
		while (matcher.find()) {
			firstNum = Integer.parseInt(matcher.group("first"));

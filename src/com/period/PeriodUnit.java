...
public class PeriodUnit {
	...
	public PeriodUnit(TimeUnit timeStart, TimeUnit timeEnd, TimeNormalizer timeNormalizer) {
		...
		if ((startNorm.charAt(startNorm.length() - 1) == '月' || startNorm.charAt(startNorm.length() - 1) == '日')
				&& endNorm.charAt(endNorm.length() - 1) == '年'
				&& compareYear(timeStart.getTime(), timeEnd.getTime()) == 0) {
			start = timeStart;
			end = timeEnd;

		} else if (startNorm.charAt(startNorm.length() - 1) == '日' && endNorm.charAt(endNorm.length() - 1) == '月'
				&& compareMonth(timeStart.getTime(), timeEnd.getTime()) == 0) {
			start = timeStart;
			end = timeEnd;
		} else {
			if (timeStart.getTime().after(timeEnd.getTime())) {
				// 检验时间是否进行过修正
				if (timeStart.isModified()) {
					if (!timeEnd.isModified()) {
						String modifiedType = timeStart.modifiedType;
						timeEnd.modifyUnit(modifiedType);
					}
					start = timeStart;
					end = timeEnd;
				} else if (timeEnd.isModified()) {
					if (!timeStart.isModified()) {
						String modifiedType = timeEnd.modifiedType;
						timeStart.modifyUnit(modifiedType);
					}
					start = timeStart;
					end = timeEnd;
				} else {
	...
	private void judgeSetFuture(boolean setEnd) {
		...
		if (lastChar == '月') {
			...
			if (setEnd) {
				end = new TimeUnit(formatDate(calendar.getTime()), timeNormalizer, start._tp);
	 ...
	 * @author LinZheng Chai
	 * @date 2019/6/4 19:43
	 * @param: timeLen 向前数的天数, measure 单位(年月日周)
	 * @return
	...
	private void setTimeEnd() {
		...
		if (timeAcc == TimeAcc.year) {
			...
			if (compareYear(start.getTime(), new Date()) == 0 && compareMonth(start.getTime(), new Date()) == 0) {
				end = new TimeUnit("今天", timeNormalizer);
			} else {
				calendar.add(Calendar.MONTH, 1);
				calendar.add(Calendar.DAY_OF_MONTH, -1);
			...
			end = new TimeUnit(formatDate(endDay), timeNormalizer);
	...
	public String toString() {
		...
		return DateUtil.formatDateDefault(start.getTime()) + " - " + DateUtil.formatDateDefault(end.getTime()) + "\n";
